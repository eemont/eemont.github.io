      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-zinc-950/80 backdrop-blur">
        ...
        <div className={`md:hidden overflow-hidden transition-all duration-300 ${open ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}>
          <nav className="flex flex-col gap-1 px-4 pb-4 text-sm text-zinc-300">
            ...
            <button
              className="mt-1 text-left rounded-lg border border-white/10 px-3 py-2 hover:border-white/20 hover:bg-white/5 hover:text-white transition-all duration-200"
              onClick={() => { setResumeOpen(true); setOpen(false); }}
            >
              Resume
            </button>
          </nav>
